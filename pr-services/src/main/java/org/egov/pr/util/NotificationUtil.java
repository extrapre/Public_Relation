...
import static org.egov.pr.util.PrConstants.*;
...
public class NotificationUtil {
	...
	@Autowired
	public NotificationUtil(PrConfiguration config, ServiceRequestRepository serviceRequestRepository,
			Producer producer) {
		this.config = config;
	...
	/**
	 * Creates customized message based on PublicRelation
	 * 
	 * @param license
	 *            The PublicRelation for which message is to be sent
	 * @param localizationMessage
	 *            The messages from localization
	 * @return customized message based on PublicRelation
	 */
	public String getCustomizedMsg(RequestInfo requestInfo, PublicRelationEvent license, String localizationMessage) {
		String message = null, messageTemplate;
		String ACTION_STATUS = license.getAction() + "_" + license.getStatus();
		switch (ACTION_STATUS) {

		case ACTION_STATUS_INITIATED:
			messageTemplate = getMessageTemplate(PrConstants.NOTIFICATION_INITIATED, localizationMessage);
			message = getInitiatedMsg(license, messageTemplate);
			break;

		case ACTION_STATUS_APPLIED:
			messageTemplate = getMessageTemplate(PrConstants.NOTIFICATION_APPLIED, localizationMessage);
		...
		case ACTION_STATUS_APPROVED:
			...
			messageTemplate = getMessageTemplate(PrConstants.NOTIFICATION_APPROVED, localizationMessage);
			message = getApprovedMsg(license, amountToBePaid, messageTemplate);
			break;

		case ACTION_STATUS_REJECTED:
			messageTemplate = getMessageTemplate(PrConstants.NOTIFICATION_REJECTED, localizationMessage);
			message = getRejectedMsg(license, messageTemplate);
			break;

		case ACTION_STATUS_FIELDINSPECTION:
			messageTemplate = getMessageTemplate(PrConstants.NOTIFICATION_FIELD_INSPECTION, localizationMessage);
			message = getFieldInspectionMsg(license, messageTemplate);
			break;

		case ACTION_CANCEL_CANCELLED:
			messageTemplate = getMessageTemplate(PrConstants.NOTIFICATION_CANCELLED, localizationMessage);
	...
	public StringBuilder getUri(String tenantId, RequestInfo requestInfo) {
		...
		uri.append(config.getLocalizationHost()).append(config.getLocalizationContextPath())
				...
				.append("&tenantId=").append(tenantId).append("&module=").append(PrConstants.MODULE);
	 ...
	 * 
	 * @param tenantId
	 *            tenantId of the PublicRelation
	 * @param requestInfo
	 ...
	 */
	private String getInitiatedMsg(PublicRelationEvent license, String message) {
	 ...
	 */
	private String getAppliedMsg(PublicRelationEvent license, String message) {
	 ...
	 */
	private String getApprovalPendingMsg(PublicRelationEvent license, String message) {
		// message = message.replace("<1>",);
	 ...
	 */
	private String getApprovedMsg(PublicRelationEvent license, BigDecimal amountToBePaid, String message) {
	 ...
	 */
	private String getRejectedMsg(PublicRelationEvent license, String message) {
		// message = message.replace("<1>",);
	 ...
	 * @return customized message for rejected
	 */
	private String getFieldInspectionMsg(PublicRelationEvent license, String message) {
	 ...
	 */
	private String getCancelledMsg(PublicRelationEvent license, String message) {
	 ...
	 */
	public String getOwnerPaymentMsg(PublicRelationEvent license, Map<String, String> valMap, String localizationMessages) {
		String messageTemplate = getMessageTemplate(PrConstants.NOTIFICATION_PAYMENT_OWNER, localizationMessages);
	 ...
	 */
	public String getPayerPaymentMsg(PublicRelationEvent license, Map<String, String> valMap, String localizationMessages) {
		String messageTemplate = getMessageTemplate(PrConstants.NOTIFICATION_PAYMENT_PAYER, localizationMessages);
		messageTemplate = messageTemplate.replace("<2>", valMap.get(amountPaidKey));
	 ...
	 *            The RequestInfo of the request
	 * @param license
	 *            The PublicRelation object for which
	 * @return
	 */
	private BigDecimal getAmountToBePaid(RequestInfo requestInfo, PublicRelationEvent license) {
	 ...
	 * 
	 * @param license
	 *            The PublicRelation for which getBill has to be called
	 * @return The uri for the getBill
	 */
	private StringBuilder getBillUri(PublicRelationEvent license) {
		return null;
	 ...
	 * 
	 * @param message
	 *            The message for the specific PublicRelation
	...
	public String getCustomizedMsg(Difference diff, PublicRelationEvent license, String localizationMessage) {
		String message = null, messageTemplate;
		...
		if (!CollectionUtils.isEmpty(diff.getFieldsChanged()) || !CollectionUtils.isEmpty(diff.getClassesAdded())
			...
			messageTemplate = getMessageTemplate(PrConstants.NOTIFICATION_OBJECT_MODIFIED, localizationMessage);
	 ...
	 */
	private String getEditMsg(PublicRelationEvent license, List<String> list, String message) {
	...
	private String getEditMsg(PublicRelationEvent license, String message) {
		message = message.replace("<APPLICATION_NUMBER>", license.getApplicationNumber());
		return message;
	}
